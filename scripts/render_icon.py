"""scripts/render_icon.py — render the OPYT tile mark to every raster the repo ships.

Two outputs, one mark: website/favicon.ico (16/32/48, the browser favicon) and
website/icon.png (512px, the X avatar). Both are checked in; re-run this by hand when the
mark changes.

Until 2026-09-14 the PNG was mcpb/icon.png, the Claude Desktop extension icon, and this file
lived in mcpb/. The .mcpb bundle was deleted that day (see
docs/plans/2026-09-14-the-install-must-carry-fixes.md); this script survived it because the
favicon never had anything to do with the extension.
...
polygon at 8x supersample.
    ...
    python scripts/render_icon.py     # needs Pillow
"""
