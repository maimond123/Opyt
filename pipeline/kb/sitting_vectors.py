...
cosine against a seed), but `sitting_zoom.py`'s `zoom` and `sitting_scheduler.py` need the
...
# because the WHERE clauses below and the sitting siblings read it as `sv.HUMAN_ATTESTED`
# (sitting_store, sitting_reader, sitting_builder, sitting_scheduler).
#
# CORRECTED 2026-09-05: this named `retrieve.py` as a reader. It reads `schema.HUMAN_ATTESTED`
# and has never imported this module — the leaf invariant above is why it cannot.
...
def _human_clause(modes: tuple[str, ...] = HUMAN_ATTESTED) -> str:
    """The atoms table is aliased `a` at all seven call sites, so the alias is not a parameter.
    `modes` is, because `_relevance` passes `REGION_VISIBLE` where everyone else takes the default.
    """
    return f"a.entry_mode IN ({_in_clause(len(modes))})"
...
def _relevance(conn, seeds: np.ndarray, *, restrict: set[str] | None = None,
    ...
    ONE streaming pass, several seeds at a time — `zoom` stacks one centroid per sub-region and
    `sitting_scheduler` stacks one anchor per region, so both would otherwise read the same blobs
    once per seed.
